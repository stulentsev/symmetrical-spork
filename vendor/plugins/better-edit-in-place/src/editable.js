...
var Editable = Class.create({
    // If you need to modify the message, change also string in init.rb
    ...
    initialize: function(element, options) {
        ...
        var opts = this.element.readAttribute('rel').evalJSON();
        this.element.setAttribute('rel_url', opts['url']);

        // Set default values for options
        this.editField = this.editField || {};
        this.editField.type = this.editField.type || opts['type'] || 'input';
        ...
        if(this.value == this.default_text)
            this.element.writeAttribute('class',  'editable_empty');
    ...
    setupForm: function() {
        this.editForm = new Element('form', {
            'action': this.element.readAttribute('rel_url'),
            'style':'display:none',
        ...
        var token_val = window._token;
        this.tokenField = new Element('input', {
            type: 'hidden',
        ...
        this.editForm.insert(this.tokenField);
    ...
    setupInputElement: function() {
        ...
        if(this.editField.type == 'select') {
            ...
            if(this.element.innerHTML != this.default_text)
                this.editField.element.value = this.element.innerHTML;
            else
                this.editField.element.value = '';

            // Copy attributes from the source element
            for(var i = 0; i < this.element.attributes.length; ++i)
            {
                var kv = this.element.attributes[i];
                if(kv.name == 'id'
                        || kv.name == 'name'
                        || kv.name == 'class'
                        || kv.name == 'style')
                    continue;
                this.editField.element.writeAttribute(kv.name, kv.value)
            }
    ...
    save: function(event) {
        ...
        new Ajax.Request(url + ".json", {
            ...
            onSuccess: function(transport) {
                ...
                this.value = value == ''? this.default_text : value;
                this.editField.element.value = this.value;
                this.element.update(this.value);

                this.element.writeAttribute('class', this.value == this.default_text
                                                          ? 'editable_empty'
                                                          : 'editable' );
    ...
    cancel: function(event) {
        this.element.show();
        this.editField.element.value = this.value == this.default_text ? '' : this.value;
        this.editForm.hide();
        if (event) {
            event.stop();
        }
